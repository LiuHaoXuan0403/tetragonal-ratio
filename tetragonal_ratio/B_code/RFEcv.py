...
def exhaustive(features_list):
    ...
    for i in range(len(features_list)-1,len(features_list)):
...
#    train_scores
                ...
                train_scores_list = {'nfeatures':number_of_features,
                ...
                train_scores_list.to_csv('./train/%d.csv' %number_of_features,sep=',')
#cv_scores
                cv_R2_list.append(cv_R2)
                cv_RMSE_list.append(cv_RMSE)
                cv_scores_list = {'nfeatures':number_of_features,
                                      'cv_R2':cv_R2_list,
                                    'cv_RMSE':cv_RMSE_list,
                                    'columns':columns_list
                                 }
                cv_scores_list=pd.DataFrame(cv_scores_list)
                cv_scores_list.to_csv('./cv/%d.csv' %number_of_features,sep=',')
    index=(cv_scores_list['cv_RMSE'].idxmin())
    features_list_update=cv_scores_list.iloc[index,3]
#    print(features_list_update)
    return features_list_update      
if __name__ == '__main__':
      
    df=readdata('./df_afterpearson.csv')
    features_list=readdata('./features_after_mine.csv')
    features_list=features_list.columns.tolist()
#    print(cv_scores_list.iloc[index,8])
#    print(features_list)
    for i in range(1,len(features_list)):
          print(i)
          features_list_update=exhaustive(features_list)
          features_list=features_list_update
